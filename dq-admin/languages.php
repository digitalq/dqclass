<?php if ( ! defined('ABS_PATH')) exit('ABS_PATH is not loaded. Direct access is not allowed.');
        ...
        function doModel()
        {
            switch ($this->action) {
                ...
                case('enable_bo_selected'):
                                            osc_csrf_check();
                                            $msg      = _m('Selected languages have been enabled for the backoffice (dq-admin)');
                                            $iUpdated = 0;
                ...
                case('disable_bo_selected'):
                                            osc_csrf_check();
                                            $msg         = _m('Selected languages have been disabled for the backoffice (dq-admin)');
                                            $msg_warning = '';
                                            $iUpdated    = 0;
                ...
                default:
                                            ...
                                            for($i = $start; $i < $max; $i++) {
                                                ...
                                                $options[] = '<a href="' . osc_admin_base_url(true) . '?page=languages&amp;action=' . ( $l['b_enabled_bo'] == 1 ? 'disable_bo_selected' : 'enable_bo_selected' ) . '&amp;id[]=' . $l['pk_c_code'] . '&amp;' . osc_csrf_token_url() . '">' . ( $l['b_enabled_bo'] == 1 ? __('Disable (dq-admin)') : __('Enable (dq-admin)') ) . '</a>';
                                            ...
                                            $bulk_options = array(
                                                ...
                                                array('value' => 'enable_bo_selected', 'data-dialog-content' => sprintf(__('Are you sure you want to %s the selected languages?'), strtolower(__('Enable (dq-admin)'))), 'label' => __('Enable (dq-admin)')),
                                                array('value' => 'disable_bo_selected', 'data-dialog-content' => sprintf(__('Are you sure you want to %s the selected languages?'), strtolower(__('Disable (dq-admin)'))), 'label' => __('Disable (dq-admin)')),
    ...
    /* file end: ./dq-admin/languages.php */
?>
