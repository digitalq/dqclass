...
function richedit_load_js()
{
    ...
    if( ($location=='item' && ($section=='item_add' || $section=='item_edit')) ||
        ...
        ?>
        <script type="text/javascript" src="<?php echo osc_base_url().'dq-content/plugins/'.osc_plugin_folder(__FILE__);?>tiny_mce/tiny_mce.js"></script>
        <script type="text/javascript">
            tinyMCE.init({
                mode : "none",
...
function rich_edit_init() {
    ...
    if(($location=='item' && ($section=='item_add' || $section=='item_edit')) || ($location=='items' && ($section=='post' || $section=='item_edit'))) {
        if(osc_version()>=310) {
            osc_register_script('tiny_mce', osc_base_url().'dq-content/plugins/'.osc_plugin_folder(__FILE__).'tiny_mce/tiny_mce.js');
