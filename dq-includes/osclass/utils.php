...
function osc_plugin_path($file) {
    ...
    $file = $plugin_path . preg_replace('#^.*dq-content\/plugins\/#','',$file);
    return $file;
}
...
function osc_plugin_url($file) {
    ...
    $dir = osc_base_url() . 'dq-content/plugins/' . preg_replace('#^.*dq-content\/plugins\/#','',$dir) . "/";
    return $dir;
}
...
function osc_plugin_folder($file) {
    // Sanitize windows paths and duplicated slashes
    $dir = preg_replace('|/+|','/', str_replace('\\','/',dirname($file)));
    $dir = preg_replace('#^.*dq-content\/plugins\/#','',$dir) . "/";
    return $dir;
}
        ...
        while (($file = readdir($dh)) !== false) {
            if($file!="." && $file!="..") {
                if(is_dir(str_replace("//", "/", $dir . "/" . $file))) {
                    if(str_replace("//", "/", $dir)==(ABS_PATH . "dq-content/themes")) {
                        if($file=="modern" || $file=="index.php") {
                            $res = osc_check_dir_writable( str_replace("//", "/", $dir . "/" . $file));
                            if(!$res) { return false; };
                        }
                    } else if(str_replace("//", "/", $dir)==(ABS_PATH . "dq-content/plugins")) {
                        if($file=="google_maps" || $file=="google_analytics" || $file=="index.php") {
                            $res = osc_check_dir_writable( str_replace("//", "/", $dir . "/" . $file));
                            if(!$res) { return false; };
                        }
                    } else if(str_replace("//", "/", $dir)==(ABS_PATH . "dq-content/languages")) {
                        if($file=="en_US" || $file=="index.php") {
                            $res = osc_check_dir_writable( str_replace("//", "/", $dir . "/" . $file));
                            if(!$res) { return false; };
                        }
                    } else if(str_replace("//", "/", $dir)==(ABS_PATH . "dq-content/downloads")) {
            ...
            if ($res == 1) { // Everything is OK, continue
                ...
                if ($handle = opendir($tmp_path)) {
                    ...
                    while (false !== ($_file = readdir($handle))) {
                        if ($_file != '.' && $_file != '..' && $_file != 'dq-content') {
                    ...
                    if ($fail == 0) { // Everything is OK, continue
                        ...
                        if ($error_queries[0]) { // Everything is OK, continue
                            ...
                            if ($rm_errors == 0) {
                                ...
                                $message = __('Nearly everything looks good! Your Osclass installation is up-to-date, but there were some errors removing temporary files. Please manually remove the "dq-content/downloads/oc-temp" folder');
            ...
            if ($result) { // Everything is OK, continue
                ...
                if ($res == 1) { // Everything is OK, continue
                    ...
                    if ($handle = opendir(osc_content_path() . 'downloads/oc-temp')) {
                        $folder_dest    = ABS_PATH . "dq-content/".$folder;
                        ...
                        if ($fail == 0) { // Everything is OK, continue
                            ...
                            if ($rm_errors == 0) {
                                ...
                                $message = __('Nearly everything looks good! but there were some errors removing temporary files. Please manually remove the \"dq-content/downloads/oc-temp\" folder');
                                $error = 6; // Some errors removing files
                            }
                        } else {
                            $message = __('Problems when copying files. Please check your permissions. ');
